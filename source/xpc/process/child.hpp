#ifndef _XPC_PROCESS_CHILD_HPP
#define _XPC_PROCESS_CHILD_HPP
...
public:
   ...
   */
  inline constexpr bool kill(const $::Signal::Code &signal = Signal::KILL) { return m_signal(signal); }
   ...
   * @param duration                  Wait duration/timestamp.
   */
  inline constexpr int32_t wait() { return m_wait(); }
  inline constexpr int32_t wait(const $::Chrono::Point &timestamp) { return m_wait(timestamp); }
  inline constexpr int32_t wait(const $::Chrono::Duration &duration) { return m_wait(duration); }
...
private:
   ...
   * @param signal                    Signal to send.
   */
  bool m_signal(const $::Signal::Code &signal);
   ...
   */
  int32_t m_wait();
  int32_t m_wait(const $::Chrono::Point &timestamp);
  int32_t m_wait(const $::Chrono::Duration &duration);
};
