...
void XPC::Core::Child::m_initialize(const $::String::Buffer& program, const Shell::Options& options) {
    ...
    for (const auto& arg : $::Filter(argv)) ::free(arg);
    for (const auto& kvp : $::Filter(envp)) ::free(kvp);
}

bool XPC::Core::Child::m_poll() {
...
XPC::Core::Identifier XPC::Core::Child::m_ppid() const {
#if $_PLATFORM_LINUX
    ...
    $_UNUSED $_AUTO = $::Functor::Defer([fp] { ::fclose(fp); });
...
bool XPC::Core::Child::m_signal(const XSIO::Signal::Code& signal) {
    return m_errc == INT32_MIN && ::kill(m_pid, signal.underlying()) == 0;
}
