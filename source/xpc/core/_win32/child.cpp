...
void XPC::Core::Child::m_initialize(const $::String::Buffer& program, const Shell::Options& options) {
    ...
    auto argv = $::Convert::join(options.argv, " ");
    ...
    auto success = ::CreateProcessA(static_cast<LPCSTR>(program.c_str()), static_cast<LPSTR>(argv.data()), NULL, NULL,
        true, flags, static_cast<LPVOID>(envp.data()), cwd, &startup, &m_info);
