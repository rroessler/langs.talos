...
namespace XPC::Shell {
    ...
    /// @brief The "which" program handler.
    using Which = $::Functor::Shared<decltype(Path::resolve)>;

    /// @brief Variadic Argument Collection.
    using Arguments = std::vector<$::String::Buffer>;

    /// @brief Command Environment Mapping.
    using Environment = $::Dict<$::String::Buffer>;
    ...
    struct Options {
        ...
        /// @brief A program resolver handler.
        Which which = Path::resolve;

        /// @brief The given environment to inherit.
        Environment envp = $::Environment::view();
    };
    ...
    class Builder {
       ...
       public:
        ...
        inline constexpr const Arguments& argv() const noexcept { return m_options.argv; }
        inline constexpr Builder& argv(const Arguments& argv) noexcept {
            return $::Ranges::Append(m_options.argv, argv), *this;
        }

        inline constexpr const Environment& envp() const noexcept { return m_options.envp; }
        inline constexpr Builder& envp(const Environment& envp) noexcept { return m_options.envp = envp, *this; }
        ...
        inline constexpr Builder& which(Which&& which) noexcept { return m_options.which = which, *this; }
        inline constexpr std::optional<$::String::Buffer> which(const $::String::Buffer& path) const noexcept {
            return m_options.which(path, std::nullopt);
        }
    };
