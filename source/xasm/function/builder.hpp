...
namespace XASM::Function {
    ...
    template <class R, class... As, class... Ts>
    inline constexpr Signature Build(Ts&&... args) {
        return Signature::build<R, As...>(std::forward<Ts>(args)...);
    }

    /// @brief Constructs function signatures.
    template <class R, class... As, class... Ts>
    inline constexpr Node* Build(Compiler::Universal* compiler, Ts&&... args) {
        return compiler->add_func(Build<R, As..., Ts...>(std::forward<Ts>(args)...));
    }

    /// @brief Handles invoking callbacks.
    template <class R, class... As>
    inline constexpr Callee* Invoke(Compiler::Universal* compiler, R (*callback)(As...)) {
