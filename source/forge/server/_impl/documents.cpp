...
void Forge::Server::Events::on_document_opened(const XLSP_NOTIFICATION(DOCUMENT_OPENED) & params) {
    on_document_changed(XLSP_NOTIFICATION(DOCUMENT_CHANGED)(params.document.identifier()));
}

void Forge::Server::Events::on_document_closed(const XLSP_NOTIFICATION(DOCUMENT_CLOSED) & params) {
    auto* documents = m_connection->service<Document::Service>();
    documents->remove(params.identifier.resource);  // remove now
}

void Forge::Server::Events::on_document_changed(const XLSP_NOTIFICATION(DOCUMENT_CHANGED) & params) {
