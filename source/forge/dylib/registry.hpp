#ifndef _FORGE_DYLIB_REGISTRY_HPP
#define _FORGE_DYLIB_REGISTRY_HPP
...
#define FORGE_MM_DYLIB_UNWRAP(N, ...) \
    exports.fields().emplace(#N, Member::Factory::native(isolate, m_##N, m_name(), #N));

/// @brief Exposes a dynamic library addon.
#define FORGE_MM_DYLIB_ADDON(P, L, ...)                                   \
    static $_AUTO = Forge::Dylib::Registry::install<Forge::Package::P>(); \
    Forge::Package::P::P(Runtime::Isolate* isolate, Dylib::Exports& exports) { L(FORGE_MM_DYLIB_UNWRAP) }

/// @brief Exposes a dynamic library method.
#define FORGE_MM_DYLIB_METHOD(P, N, I, A, ...)  \
    Forge::Value::Any Forge::Package::P::m_##N( \
