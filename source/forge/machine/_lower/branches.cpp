...
$_FWD(void interrupted(Builder*), Forge::Machine::Template)
$_FWD(uint64_t interrupted(Runtime::Isolate*), Forge::Machine::Dispatch)

//  PUBLIC METHODS  //

uint64_t Forge::Machine::Dispatch::interrupted(Runtime::Isolate* isolate) {
    auto* frame = isolate->frame();  // get frame
    return $_ASSERT(frame), frame->interrupted();
}

void Forge::Machine::Template::interrupted(Builder* builder) {
...
FORGE_MM_MACHINE_EMIT(JUMP_TO, builder, instruction) {
...
FORGE_MM_MACHINE_EMIT(JUMP_FILLED, builder, instruction) {
    // attempt resolving the incoming label
...
FORGE_MM_MACHINE_EMIT(JUMP_TRUTHY, builder, instruction) {
    // resolve the necessary incoming register
    auto tv = instruction->get<1>();
...
FORGE_MM_MACHINE_EMIT(JUMP_FALSEY, builder, instruction) {
    // resolve the necessary incoming register
    auto tv = instruction->get<1>();
