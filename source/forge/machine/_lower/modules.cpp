...
$_FWD(Reference open(Runtime::Isolate*), Forge::Machine::Dispatch)
$_FWD(Reference close(Runtime::Isolate*), Forge::Machine::Dispatch)
$_FWD(Reference import(Runtime::Isolate*, const String::Intern*), Forge::Machine::Dispatch)
$_FWD(Reference expose(Runtime::Isolate*, Reference, const String::Intern*), Forge::Machine::Dispatch)

//  PUBLIC METHODS  //

Forge::Machine::Reference Forge::Machine::Dispatch::open(Runtime::Isolate* isolate) {
    const auto* frame = isolate->frame()->as<Machine::Frame>();
    return isolate->exports(frame->resource())->open(isolate).pointer();
}

Forge::Machine::Reference Forge::Machine::Dispatch::close(Runtime::Isolate* isolate) {
    const auto* frame = isolate->frame()->as<Machine::Frame>();
    return isolate->exports(frame->resource())->close().pointer();
}

Forge::Machine::Reference Forge::Machine::Dispatch::import(Runtime::Isolate* isolate, const String::Intern* intern) {
...
Forge::Machine::Reference Forge::Machine::Dispatch::expose(
...
FORGE_MM_MACHINE_EMIT(MODULE_OPEN, builder, instruction) {
    auto dx = __ee__ resolve(instruction->get<0>());
    __ee__ invoke(Dispatch::open, dx, builder->isolate);
}

FORGE_MM_MACHINE_EMIT(MODULE_CLOSE, builder, instruction) {
    auto dx = __ee__ resolve(instruction->get<0>());
    __ee__ invoke(Dispatch::close, dx, builder->isolate);
}

FORGE_MM_MACHINE_EMIT(MODULE_IMPORT, builder, instruction) {
...
FORGE_MM_MACHINE_EMIT(MODULE_EXPORT, builder, instruction) {
