#ifndef _FORGE_VALUE_SYMBOL_HPP
#define _FORGE_VALUE_SYMBOL_HPP
...
template <>
struct ankerl::unordered_dense::hash<Forge::Value::Symbol> {
    using is_avalanching = void;  // declare that this is not avalanchable
    $_NODISCARD uint64_t operator()(const Forge::Value::Symbol& symbol) const noexcept { return symbol.hash(); }
};
