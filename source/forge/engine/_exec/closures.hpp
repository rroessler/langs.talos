...
$_INLINE_FORCE Forge::Engine::Mode Forge::Engine::Dispatch::m_recall(
...
$_INLINE_FORCE Forge::Engine::Mode Forge::Engine::Dispatch::m_invoke(
...
$_INLINE_FORCE Forge::Engine::Mode Forge::Engine::Dispatch::m_spawn(
    Runtime::Isolate* isolate, Function::Frame* frame, Register sink, Value::Any target, const Arguments& args) {
...
FORGE_MM_ENGINE_EXECUTE(CLOSURE_MAKE, isolate, frame, instruction) {
...
FORGE_MM_ENGINE_EXECUTE(CLOSURE_PASS, isolate, frame, instruction) {
...
FORGE_MM_ENGINE_EXECUTE(CALL_0_VOID, isolate, frame, instruction) {
    return m_invoke(isolate, frame, instruction->get<0>(), frame->accumulator());
}

FORGE_MM_ENGINE_EXECUTE(CALL_N_VOID, isolate, frame, instruction) {
    auto [callee, argv] = frame->split(instruction->get<1>());
    return m_invoke(isolate, frame, instruction->get<0>(), callee, argv);
}

FORGE_MM_ENGINE_EXECUTE(CALL_0_INLINE, isolate, frame, instruction) {
    return m_recall(isolate, frame, instruction->get<0>());
}

FORGE_MM_ENGINE_EXECUTE(CALL_N_INLINE, isolate, frame, instruction) {
    return m_recall(isolate, frame, instruction->get<0>(), frame->span(instruction->get<1>()));
}

FORGE_MM_ENGINE_EXECUTE(CALL_0_FIELD, isolate, frame, instruction) {
...
FORGE_MM_ENGINE_EXECUTE(CALL_N_FIELD, isolate, frame, instruction) {
...
FORGE_MM_ENGINE_EXECUTE(SPAWN_0_VOID, isolate, frame, instruction) {
    return m_spawn(isolate, frame, instruction->get<0>(), frame->accumulator());
}

FORGE_MM_ENGINE_EXECUTE(SPAWN_N_VOID, isolate, frame, instruction) {
    auto [callee, argv] = frame->split(instruction->get<1>());
    return m_spawn(isolate, frame, instruction->get<0>(), callee, argv);
}

FORGE_MM_ENGINE_EXECUTE(SPAWN_0_FIELD, isolate, frame, instruction) {
    auto target = frame->accumulator();  // get the baseline target
    auto callee = m_getter(isolate, frame, target, instruction->get<1>());
    if (!callee.traits().okay()) return Mode::PANIC;  // fail early if possible
...
FORGE_MM_ENGINE_EXECUTE(SPAWN_N_FIELD, isolate, frame, instruction) {
    auto [target, argv] = frame->split(instruction->get<2>());
    auto callee = m_getter(isolate, frame, target, instruction->get<1>());
    if (!callee.traits().okay()) return Mode::PANIC;  // failed calling so exit now
