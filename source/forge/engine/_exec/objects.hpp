...
$_INLINE_FORCE Forge::Value::Any Forge::Engine::Dispatch::m_object(
...
FORGE_MM_ENGINE_EXECUTE(OBJECT_EMPTY, isolate, frame, instruction) {
    auto object = isolate->create<Object::Instance>();  // construct
    return frame->store(instruction->get<0>(), object), Mode::NEXT;
}

FORGE_MM_ENGINE_EXECUTE(OBJECT_MAKE, isolate, frame, instruction) {
