...
/// Dispatch Modules
#include "forge/engine/_inline/execute.ipp"

//  PUBLIC METHODS  //

Forge::Pointer::Underlying Forge::Engine::Dispatch::getter(
...
Forge::Pointer::Underlying Forge::Engine::Dispatch::setter(
...
Forge::Engine::Subtype Forge::Engine::Dispatch::extends(Pointer::Underlying value, Pointer::Underlying guard) {
    return m_extends(Value::Any(value), Value::Any(guard));
}

Forge::Pointer::Underlying Forge::Engine::Dispatch::ensure(
...
Forge::Pointer::Underlying Forge::Engine::Dispatch::concat(
...
Forge::Pointer::Underlying Forge::Engine::Dispatch::list(Isolate* isolate, const Function::Arguments& args) {
    return m_list(isolate, args.span()).pointer();
}

Forge::Pointer::Underlying Forge::Engine::Dispatch::object(Isolate* isolate, const Function::Arguments& args) {
    return m_object(isolate, args.span()).pointer();
}

Forge::Pointer::Underlying Forge::Engine::Dispatch::iterator(Isolate* isolate, Pointer::Underlying iterable) {
    return m_iterator(isolate, Value::Any(iterable)).pointer();
}

Forge::Pointer::Underlying Forge::Engine::Dispatch::enumeration(Isolate* isolate, const Function::Arguments& args) {
    return m_enumeration(isolate, args.span()).pointer();
}

//  PRIVATE METHODS  //

Forge::Value::Any Forge::Engine::Dispatch::m_getter(
...
Forge::Value::Any Forge::Engine::Dispatch::m_setter(
...
void Forge::Engine::Dispatch::m_feedback(Isolate* isolate, const Feedback& feedback) {
...
Forge::Value::Any Forge::Engine::Dispatch::m_execute(Isolate* isolate, Function::Frame* frame) {
    // ensure the frames currently match
    $_ASSERT(isolate->frame() == frame, "Mismatched VM frames");

    // prepare the available dispatch table
    static void* s_table[] = {
#define FORGE_XX_SYLLABLE_BASE(N, ...) &&_DISPATCH_##N,
