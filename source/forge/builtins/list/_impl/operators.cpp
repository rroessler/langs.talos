...
struct FORGE_BUILTIN_OPERATORS(Iterable::List) {
    static Value::Any iterator(Runtime::Isolate* isolate, Iterable::List self);
};

//  PROPERTIES  //

Forge::Operator::Storage<Forge::Iterable::List> FORGE_BUILTIN_TRAITS(
    Iterable::List)::m_operators = Operator::Storage<Iterable::List>({ { Operator::Kind::ITER, Apply::iterator } });

//  PUBLIC METHODS  //

FORGE_MM_BUILTIN_OPERATOR(Iterable::List, iterator, isolate, Iterable::List self) {
...
Forge::Member::View FORGE_BUILTIN_TRAITS(Iterable::List)::m_attributes(const Iterable::List&, Operator::Kind kind) {
