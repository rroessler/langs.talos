...
Forge::Type::Erased FORGE_BUILTIN_TRAITS(Object::Exception)::typing() { return prototype()->instantiate(); }

FORGE_MM_BUILTIN_FTYPE(Object::Exception, name) { return { TB::function(TB::string()) }; }
FORGE_MM_BUILTIN_FTYPE(Object::Exception, message) { return { TB::function(TB::string()) }; }

FORGE_MM_BUILTIN_STYPE(Object::Exception, from) {
    auto format = TB::optional(TB::string());
    auto args = TB::arguments(format, TB::any());
    return { TB::variadic(typing(), args) };
}

FORGE_MM_BUILTIN_STYPE(Object::Exception, named) {
...
void FORGE_BUILTIN_TRAITS(Object::Exception)::m_typedefs(Type::World* globals) {
    // prepare the prototype to be constructed
    auto proto = prototype();

#define X(N, ...) { #N, Field::N() },
    proto->fields() = $::Record<Type::Entity>({ FORGE_XX_FIELDS_EXCEPTION(X) });
#undef X

#define X(N, ...) { #N, Static::N() },
    proto->statics() = $::Record<Type::Entity>({ FORGE_XX_STATICS_EXCEPTION(X) });
#undef X
