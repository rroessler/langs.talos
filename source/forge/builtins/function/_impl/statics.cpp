...
FORGE_MM_BUILTIN_STATIC(Function::Dynamic, limit, , ) { return Number::Tagged(UINT64_MAX); }

FORGE_MM_BUILTIN_STATIC(Function::Dynamic, call, isolate, args) {
...
FORGE_MM_BUILTIN_STATIC(Function::Dynamic, apply, isolate, args) {
    // ensure we have some valid values as necessary now
    FORGE_MM_ASSERT_ARGC(isolate, args.size(), 1);
    FORGE_MM_ASSERT_TYPEOF(isolate, Function::Dynamic, args[0]);
    ...
    FORGE_MM_ASSERT_TYPEOF(isolate, Iterable::List, args[1]);
    return isolate->invoke(functor, args.at<Iterable::List>(1).span());
}

//  PRIVATE METHODS  //

Forge::Value::Any FORGE_BUILTIN_TRAITS(Function::Dynamic)::m_globals(Runtime::Isolate* isolate) {
