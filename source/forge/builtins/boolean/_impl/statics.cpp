...
FORGE_MM_BUILTIN_STATIC(Value::Boolean, from, isolate, args) {
    FORGE_MM_ASSERT_ARGC(isolate, args.size(), 1);
    return Value::Boolean(args.at(0).truthiness());
}

FORGE_MM_BUILTIN_STATIC(Value::Boolean, parse, isolate, args) {
    FORGE_MM_ASSERT_ARGC(isolate, args.size(), 1);  // prepare
...
Forge::Value::Any FORGE_BUILTIN_TRAITS(Value::Boolean)::m_globals(Runtime::Isolate* isolate) {
