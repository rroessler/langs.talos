...
struct FORGE_BUILTIN_STATICS(Value::Boolean) {
    FORGE_XX_STATICS_BOOLEAN(X)
};
#undef X

//  PUBLIC METHODS  //

Forge::Type::Erased FORGE_BUILTIN_TRAITS(Value::Boolean)::typing() { return prototype()->instantiate(); }

FORGE_MM_BUILTIN_STYPE(Value::Boolean, from) { return { TB::function(TB::boolean(), TB::arguments(TB::any())) }; }
FORGE_MM_BUILTIN_STYPE(Value::Boolean, parse) {
...
void FORGE_BUILTIN_TRAITS(Value::Boolean)::m_typedefs(Type::World* globals) {
    // get the underlying prototype instance
    auto proto = prototype();

#define X(N, ...) { #N, Static::N() },
    proto->statics() = { FORGE_XX_STATICS_BOOLEAN(X) };
#undef X
