...
FORGE_BUILTIN_STORAGE(Async::Future) = Forge::Member::Storage(name(), { FORGE_XX_FIELDS_FUTURE(X) });
#undef X

//  PUBLIC METHODS  //

FORGE_MM_BUILTIN_FIELD(Async::Future, then, isolate, args) {
    // ensure some conditions about the arguments
    FORGE_MM_ASSERT_ARGC(isolate, args.size(), 1);
    FORGE_MM_ASSERT_TYPEOF(isolate, Async::Future, args.self());
    ...
    auto native = FORGE_MM_GLOBALS_NATIVE(isolate, "Future.then:callback", s_callback);

    // and construct the incoming future instance now
    return isolate->create<Async::Future>(native, args);
}

FORGE_MM_BUILTIN_FIELD(Async::Future, fails, isolate, args) {
    // ensure some conditions about the arguments
    FORGE_MM_ASSERT_ARGC(isolate, args.size(), 1);
    FORGE_MM_ASSERT_TYPEOF(isolate, Async::Future, args.self());
    FORGE_MM_ASSERT_TYPEOF(isolate, Function::Dynamic, args[0]);
    ...
    // construct the cached native to be used now
    auto native = FORGE_MM_GLOBALS_NATIVE(isolate, "Future.fails:callback", s_callback);

    // and construct the incoming future instance now
    return isolate->create<Async::Future>(native, args);
}

FORGE_MM_BUILTIN_FIELD(Async::Future, await, isolate, args) {
...
FORGE_MM_BUILTIN_FIELD(Async::Future, state, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Async::Future, args.self());
...
Forge::Member::View FORGE_BUILTIN_TRAITS(Async::Future)::m_attributes(const Async::Future&, Value::Symbol symbol) {
