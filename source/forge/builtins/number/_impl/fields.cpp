...
FORGE_BUILTIN_STORAGE(Number::Tagged) = Forge::Member::Storage(name(), { FORGE_XX_FIELDS_NUMBER(X) });
#undef X

//  PUBLIC METHODS  //

FORGE_MM_BUILTIN_FIELD(Number::Tagged, abs, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    return Number::Tagged(std::abs(args.self<Number::Tagged>().value()));
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, sign, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    auto value = args.self<Number::Tagged>().value();    // get value...
    return Number::Tagged(Number::Math::signum(value));  // ...and sign
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, ceil, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    return Number::Tagged(std::ceil(args.self<Number::Tagged>().value()));
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, floor, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    return Number::Tagged(std::floor(args.self<Number::Tagged>().value()));
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, round, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    return Number::Tagged(std::round(args.self<Number::Tagged>().value()));
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, trunc, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    return Number::Tagged(std::trunc(args.self<Number::Tagged>().value()));
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, clamp, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    ...
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, minimum);
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, maximum);
...
FORGE_MM_BUILTIN_FIELD(Number::Tagged, is_nan, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    return Value::Boolean(std::isnan(args.self<Number::Tagged>().value()));
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, is_fin, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    return Value::Boolean(std::isfinite(args.self<Number::Tagged>().value()));
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, is_inf, isolate, args) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());
    return Value::Boolean(std::isinf(args.self<Number::Tagged>().value()));
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, to_precision, isolate, args) {
    return m_stringify(isolate, args, "to_precision");
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, to_scientific, isolate, args) {
    return m_stringify(isolate, args, "to_scientific");
}

//  PRIVATE METHODS  //

Forge::Member::View FORGE_BUILTIN_TRAITS(Number::Tagged)::m_attributes(const Number::Tagged&, Value::Symbol symbol) {
    return m_members.retrieve(symbol);
}

FORGE_MM_BUILTIN_FIELD(Number::Tagged, m_stringify, isolate, args, const $::String::View& method) {
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, args.self());

    auto digits = args.at(0, Number::Tagged(0));  // fraction
    FORGE_MM_ASSERT_TYPEOF(isolate, Number::Tagged, digits);
