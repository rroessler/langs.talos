...
static auto g_E = Forge::Builtins::TB::constraint("E", Forge::Builtins::TB::any(), Forge::Builtins::TB::any());

//  PUBLIC METHODS  //

Forge::Type::Erased FORGE_BUILTIN_TRAITS(Monad::Result)::typing() {
    return TB::generic(prototype()->instantiate(), std::vector({ g_T, g_E }));
}

FORGE_MM_BUILTIN_FTYPE(Monad::Result, is_okay, const Self*) { return { TB::function(TB::boolean()) }; }
FORGE_MM_BUILTIN_FTYPE(Monad::Result, is_error, const Self*) { return { TB::function(TB::boolean()) }; }

FORGE_MM_BUILTIN_FTYPE(Monad::Result, unwrap_okay, const Self* self) {
...
FORGE_MM_BUILTIN_FTYPE(Monad::Result, unwrap_error, const Self* self) {
...
FORGE_MM_BUILTIN_STYPE(Monad::Result, wrap) {
...
FORGE_MM_BUILTIN_STYPE(Monad::Result, okay) {
...
FORGE_MM_BUILTIN_STYPE(Monad::Result, error) {
    // prepare the constraints
    auto T = TB::constraint("T", TB::any(), TB::any());
    auto E = TB::constraint("E", TB::any(), TB::any());
...
void FORGE_BUILTIN_TRAITS(Monad::Result)::m_typedefs(Type::World* globals) {
...
#define X(N, ...) { #N, Static::N() },
    proto->statics() = $::Record<Type::Entity>({ FORGE_XX_STATICS_RESULT(X) });
#undef X
