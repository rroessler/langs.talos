...
struct FORGE_BUILTIN_FIELDS(Object::Enum) {
    FORGE_XX_FIELDS_ENUM(X)
};
#undef X
...
FORGE_MM_BUILTIN_STYPE(Object::Enum, name) {
...
FORGE_MM_BUILTIN_STYPE(Object::Enum, label) {
    auto E = TB::constraint("E");
    auto arguments = TB::arguments(E, TB::number());
    auto signature = TB::function(TB::string(), arguments);
    return { TB::generic(signature, TB::parameters(E)) };
...
FORGE_MM_BUILTIN_STYPE(Object::Enum, value) {
    auto E = TB::constraint("E");
    auto arguments = TB::arguments(E, TB::number());
...
void FORGE_BUILTIN_TRAITS(Object::Enum)::m_typedefs(Type::World* globals) {
...
#define X(N, ...) { #N, Static::N() },
    proto->statics() = { FORGE_XX_STATICS_ENUM(X) };
#undef X
