...
Forge::Bundle::Worker::Worker(const Options& options) : Main(), m_options(options) {}
Forge::Bundle::Worker::Worker(XI::Container* services, const Options& options) :
    Main(services), m_archive(services), m_options(options) {}

//  PRIVATE METHODS  //

$_NORETURN void Forge::Bundle::Worker::m_execute() {
...
$::URI::Buffer Forge::Bundle::Worker::m_resolve(const $::String::View& script) {
...
$::Filesystem::Path Forge::Bundle::Worker::m_output(const $::URI::View& script) {
    return m_output(script, XJCT::Archive::Extension);
}

$::Filesystem::Path Forge::Bundle::Worker::m_output(const $::URI::View& script, const $::String::View& extension) {
...
bool Forge::Bundle::Worker::m_codesign(const $::Filesystem::Path& output) const noexcept {
    return m_archive.provider()->codesign(output) || m_failure(9000902, $::Path::relative(output).string());
}

bool Forge::Bundle::Worker::m_imbue(Executable& binary, const Buffer& buffer) const noexcept {
    return m_imbue(binary, Blob(reinterpret_cast<const char*>(buffer.data()), buffer.size()));
}

bool Forge::Bundle::Worker::m_imbue(Executable& binary, const Blob& blob) const noexcept {
