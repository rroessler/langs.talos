...
Forge::Bytecode::Compiler::Compiler() : Compiler($::Global::get<Runtime::Container>()) {}

Forge::Bytecode::Compiler::Compiler(XI::Container* services) : m_services(services) {
...
Forge::Bytecode::Index Forge::Bytecode::Compiler::constant(Value::Any value) {
...
Forge::Bytecode::Index Forge::Bytecode::Compiler::string(const $::String::View& buffer) {
...
Forge::Bytecode::Index Forge::Bytecode::Compiler::symbol(const $::String::View& buffer) {
    return constant(m_arena->strings.at(string(buffer)).symbol());
}

void Forge::Bytecode::Compiler::import(Destination sink, const $::String::View& buffer) {
...
void Forge::Bytecode::Compiler::lower(const Syntax::Node* node, Destination sink) { Visitor::visit(node, this, sink); }

void Forge::Bytecode::Compiler::preamble(const Syntax::Preamble* preamble, Register value) {
...
void Forge::Bytecode::Compiler::expose(const Syntax::Declaration* preamble, Register value) {
    if (!preamble->modifiers().test(Variable::Flag::EXPORT)) return;
    emit<Syllable::MODULE_EXPORT>(value, string(preamble->name()));
}

Forge::Bytecode::Index Forge::Bytecode::Compiler::enqueue(const Syntax::Lambda* function) const {
    return m_queue->enqueue(function, m_request ? m_request->m_variables : nullptr);
}

void Forge::Bytecode::Compiler::load(const $::String::View& name, Variable::Extent extent, Destination sink) {
...
void Forge::Bytecode::Compiler::store(const $::String::View& name, Variable::Extent extent, Register value) {
...
$::Ptr::Unique<Forge::Linker::Arena> Forge::Bytecode::Compiler::process(
...
bool Forge::Bytecode::Compiler::m_bundled() const noexcept { return m_services->exists<Bundle::Service>(); }

void Forge::Bytecode::Compiler::m_emit(const Instruction& instruction) {
...
void Forge::Bytecode::Compiler::m_function(Request* request) {
    ...
    const auto* function = request->m_function;

    // pull out the spread value as well to be compared
    const auto* spread = function->signature()->spread();
    ...
    for (const auto& [ii, parameter] : $::Each(function->parameters())) {
        if (m_parameter(parameter, spread)) info->shared()->vargs = ii;
    }

    // check if the function-body is a block or not
    Destination destination = function->body()->is<Syntax::Block>() ? Register() : Accumulator();

    // prepare the function body to be used now
    auto returns = m_labels->m_returns.emplace(m_labels->reserve());
    lower(function->body(), destination), m_labels->m_returns.pop(), m_labels->patch(returns);
...
bool Forge::Bytecode::Compiler::m_parameter(const Syntax::Variable* parameter, const Syntax::Variable* spread) {
