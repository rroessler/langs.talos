#ifndef _FORGE_BYTECODE_LABEL_HPP
#define _FORGE_BYTECODE_LABEL_HPP

/// Forge Modules
#include "forge/engine/operand.hpp"
...
template <>
struct ankerl::unordered_dense::hash<Forge::Bytecode::Index> {
    using is_avalanching = void;  // declare that this is not avalanchable
    $_NODISCARD uint64_t operator()(const Forge::Bytecode::Index& index) const noexcept { return index.encode(); }
};

template <>
struct ankerl::unordered_dense::hash<Forge::Bytecode::Label> {
    using is_avalanching = void;  // declare that this is not avalanchable
    $_NODISCARD uint64_t operator()(const Forge::Bytecode::Label& label) const noexcept { return label.encode(); }
};
