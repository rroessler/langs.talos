...
$_FWD(Value::Any fold(Operator::Kind, Value::Any, Value::Any), Forge::Bytecode::Constants)
...
FORGE_MM_LOWER_NODE(Binary, node, compiler, destination) {
    // allow access to the traits for operators
    namespace Traits = ::Forge::Operator::Traits;
    ...
    if (rv.is<Number::Tagged>()) {
        switch (opcode) {
#define FORGE_XX_SYLLABLE_INDEXED(P, N, ...)  \
...
#define FORGE_XX_TOKEN_COMPARE(N, ...) \
...
Forge::Value::Any Forge::Bytecode::Constants::fold(Operator::Kind opcode, Value::Any left, Value::Any right) {
...
Forge::Value::Any Forge::Syntax::Binary::m_fold() const noexcept {
