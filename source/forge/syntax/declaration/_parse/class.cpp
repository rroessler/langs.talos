...
Forge::Syntax::Specialization Forge::Parser::Dispatch::m_implements(Stream* parser) {
...
FORGE_MM_PARSE_DECL(Class, parser) {
    ...
    auto* base = extends ? m_expression(parser) : nullptr;
    if (base == nullptr && extends) return nullptr;
    ...
    return parser->allocate<Syntax::Class>(name->lexeme(), constructor, base, implements, *fields, location);
}
