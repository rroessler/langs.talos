...
Forge::Format::Node* Forge::Format::Dispatch::m_variable(Reader* reader, bool compressed) {
    // prepare the storage instance to be used now
    auto* storage = reader->storage();
    Node::List* variable = storage->group();

    // check for an incoming variable modifier
    auto modifier = reader->match(Lexer::Flag::VARIABLE);
...
FORGE_MM_FORMAT_HINT(Variable, reader) { return m_variable(reader); }
FORGE_MM_FORMAT_DECL(Variable, reader) { return m_variable(reader); }
