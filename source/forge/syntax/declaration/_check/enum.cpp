...
$_FWD(Erased check(Syntax::Expression*, Analyzer*, const Erased&, Diagnostic::Code), Forge::Type::Dispatch)

//  PUBLIC METHODS  //

Forge::Type::Erased Forge::Type::Dispatch::check(
...
FORGE_MM_CHECK_NODE(Enum, node, analyzer) {
...
FORGE_MM_CHECK_NODE(Variant, node, analyzer) {
    // trace this node now
    $_UNUSED $_AUTO = analyzer->trace(node);
