...
FORGE_MM_CHECK_NODE(Conditional, node, analyzer) {
    // run the baseline deduction on the node firstly
    auto deduction = analyzer->check(node->condition());
    if (deduction.type->is<Type::Failure>()) return deduction;
    ...
    auto branch = [&](const Syntax::Node* statement, bool primary) -> Type::Branch {
        ...
        else if (truthiness == primary) return [statement](auto* _) { return _->check(statement).flow; };

        analyzer->redundant(statement);  // declare as redundant now and show condition as necessary
        analyzer->report(node->condition()->traits()->bounds(), 3000600, !primary ? "truth" : "false");
