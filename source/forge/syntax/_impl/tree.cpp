...
Forge::Syntax::Tree::Tree(const $::URI::View& resource) : m_resource(resource) {
...
FORGE_MM_CHECK_NODE(Tree, tree, analyzer) {
...
FORGE_MM_LINT_NODE(Tree, tree, analyzer) {
    $_UNUSED $_AUTO = analyzer->scope();
    analyzer->verify(tree->statements(), tree);
}

FORGE_MM_LOWER_NODE(Tree, tree, compiler, ) { compiler->enqueue(tree->main()); }
