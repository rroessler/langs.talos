...
const Forge::Lexer::Token* Forge::Parser::Dispatch::m_label(Stream* parser) {
...
Forge::Syntax::Identifier* Forge::Parser::Dispatch::m_identifier(Stream* parser) {
    auto* token = m_label(parser);
    if (token == nullptr) return nullptr;
    return parser->allocate<Syntax::Identifier>(token);
}

FORGE_MM_PARSE_PREFIX(Identifier, parser, assignable) {
    ...
    switch (opcode->kind()) {
#define FORGE_XX_TOKEN_ASSIGN(K, ...)                                                                            \
