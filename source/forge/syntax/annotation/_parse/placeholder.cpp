...
std::optional<Forge::Syntax::Template> Forge::Parser::Dispatch::m_template(Stream* parser) {
...
FORGE_MM_PARSE_HINT(Placeholder, parser) {
    ...
    return parser->allocate<Syntax::Placeholder>(name->lexeme(), extends, fallback, snapshot.enclose(name));
}
