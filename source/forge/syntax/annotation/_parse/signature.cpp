...
Forge::Syntax::Signature* Forge::Parser::Dispatch::m_signature(Stream* parser, bool annotated) {
    ...
    return parser->allocate<Syntax::Signature>(base, returns, snapshot.enclose(token));
}

FORGE_MM_PARSE_HINT(Signature, parser) { return m_signature(parser, true); }
FORGE_MM_PARSE_DECL(Signature, parser) { return m_signature(parser, false); }
