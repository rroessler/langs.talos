...
bool Forge::Relint::Metadata::bound(const Syntax::Node* origin) const { return m_mirrors.contains(origin); }

const Forge::Relint::Mirror* Forge::Relint::Metadata::resolve(const Syntax::Node* origin) const {
    auto mirror = m_mirrors.find(origin);  // safe resolution
    return mirror == m_mirrors.end() ? nullptr : mirror->second.get();
}

Forge::Relint::Mirror* Forge::Relint::Metadata::resolve(const Syntax::Node* origin, const Syntax::Node* parent) {
...
const Forge::Relint::Mirror* Forge::Relint::Metadata::search(const XLSP::Position& position) const noexcept {
...
$::Ptr::Unique<Forge::Relint::Mirror> Forge::Relint::Metadata::m_create(
