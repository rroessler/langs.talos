#ifndef _FORGE_RELINT_HOOKS_HPP
#define _FORGE_RELINT_HOOKS_HPP
...
namespace Forge::Relint {
    ...
    using Delegate = $::Functor::Shared<void(const T*, Context*)>;
    ...
    class Hooks {
        ...
        $::Map<$::RTTI::Tag, std::vector<Generic>> m_delegates = {};
       ...
       public:
         ...
         */
        template <std::derived_from<Syntax::Node> T>
        inline constexpr Hooks* overload(Delegate<T>&& delegate) {
       ...
       private:
        ...
        inline constexpr const std::vector<Generic>& m_resolve(const Syntax::Node* node) {
            return m_resolve(node->traits()->tag());
        }
        ...
        inline constexpr Generic m_generic(Delegate<T>&& delegate) const noexcept {
            return [cb = std::move(delegate)](const Syntax::Node* node, Context* ctx) { cb(node->as<T>(), ctx); };
        }
    };
