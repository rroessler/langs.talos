...
$_FWD(template <class... As> Result failure(Diagnostic::Code, As&&...), Forge::Resource::Path)
...
template <class... As>
Forge::Resource::Result Forge::Resource::Path::failure(Diagnostic::Code code, As&&... args) {
    return std::unexpected(Diagnostic::Traits::format(code, std::forward<As>(args)...));
}

Forge::Resource::Result Forge::Resource::Path::resolve(const $::URI::View& resource, const $::Filesystem::Path& hint) {
#define X(N, S, ...) \
    case XH::FNV::U32(S): return resolve<Scheme::N>(resource.body(), hint);
    switch (XH::FNV::U32(resource.scheme())) {
        FORGE_XX_RESOURCE_SCHEMES(X)  // valid resource schemes
        default: return failure(8000801, resource.scheme());
    }
#undef X
}

Forge::Resource::Result Forge::Resource::Path::resolve(const $::String::View& script, const $::Filesystem::Path& hint) {
...
template <>
Forge::Resource::Result Forge::Resource::Path::resolve<Forge::Resource::Scheme::HREF>(
...
template <>
Forge::Resource::Result Forge::Resource::Path::resolve<Forge::Resource::Scheme::SCRIPT>(
...
template <>
Forge::Resource::Result Forge::Resource::Path::resolve<Forge::Resource::Scheme::ARCHIVE>(
    const $::String::View& relative, const $::Filesystem::Path&) {
    return $::URI::Buffer("arch", relative);  // should exist
}

template <>
Forge::Resource::Result Forge::Resource::Path::resolve<Forge::Resource::Scheme::DYNAMIC>(
    const $::String::View& name, const $::Filesystem::Path&) {
    return $::Trim::both(name).empty() ? failure(8000101) : $::URI::Buffer("dylib", name);
}

template <>
Forge::Resource::Result Forge::Resource::Path::resolve<Forge::Resource::Scheme::INTERNAL>(
    const $::String::View& name, const $::Filesystem::Path&) {
...
template <>
Forge::Resource::Result Forge::Resource::Path::resolve<Forge::Resource::Scheme::EXTERNAL>(
    const $::String::View& name, const $::Filesystem::Path&) {
    if ($::Trim::both(name).empty()) return failure(8000101);  // ensure the path is non-empty
