...
namespace XSIO::Task {
    ...
    template <class R, class E = std::future_error>
    requires std::is_default_constructible_v<R> class Deferred {
        //  TYPEDEFS  //
       ...
       public:
        ...
        explicit Deferred() : m_queue(m_allocate()) {}
         ...
         */
        template <class U>
        requires $::Traits::Disjunction<U, E, R> explicit Deferred(const U& value) : m_result(value) {}
         ...
         */
        template <class U>
        requires $::Traits::Disjunction<U, E, R> explicit Deferred(U&& value) : m_result(std::move(value)) {}

        /// @brief Handles safely removing deferreds.
        ~Deferred() { m_queue = m_release(m_queue); }
       ...
       private:
        ...
        inline void m_await(Virtual::Thread* thread) {
            ...
            Context::Callback callback = [&](Virtual::Thread* thread, Virtual::Processor*) {
