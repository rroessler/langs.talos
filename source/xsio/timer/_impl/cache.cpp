...
class XSIO::Timer::Awaken : public Abstract<Awaken> {
  //  PROPERTIES  //
...
public:
   ...
   */
  inline void execute(Virtual::Thread *, Virtual::Processor *processor) final {
    m_thread->awaken(), m_scheduler->schedule(m_thread, processor);
  }
};

/// @brief Handles executing callbacks.
class XSIO::Timer::After : public Abstract<After> {
  //  PROPERTIES  //
...
public:
   ...
   * @param processor             Processor to inherit.
   */
  inline void execute(Virtual::Thread *thread, Virtual::Processor *processor) final { m_callback(thread, processor); }
};
...
void XSIO::Timer::Cache::suspend(Virtual::Thread *thread, const Point &timestamp) {
  ...
  Switch::Callback callback = [timestamp, &lock](Virtual::Thread *sleeping, Virtual::Processor *processor) {
    auto *timers = processor->timers(); // get the current timers now
    auto awaken = $::Unique::New<Awaken>(sleeping, timers->m_scheduler);
...
XSIO::Timer::Token XSIO::Timer::Cache::schedule(const Point &timestamp, Callback callback) {
  ...
  return m_push(timestamp, $::Unique::New<After>(std::move(callback)));
}

XSIO::Timer::Token XSIO::Timer::Cache::schedule(const Ticks &duration, $::Unique::Pointer<Action> &&action) {
  return schedule(Timer::Point() + duration, std::move(action));
}

XSIO::Timer::Token XSIO::Timer::Cache::schedule(const Point &timestamp, $::Unique::Pointer<Action> &&action) {
  $_UNUSED $_AUTO = $::Lock::guard(m_mutex);
...
XSIO::Timer::Token XSIO::Timer::Cache::m_push(const Point &timestamp, $::Unique::Pointer<Action> &&action) {
