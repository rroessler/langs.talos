...
public:
  ...
  Token schedule(const Ticks &duration, $::Unique::Pointer<Action> &&action);
  ...
  Token schedule(const Point &timestamp, $::Unique::Pointer<Action> &&action);
...
private:
   ...
   */
  Token m_push(const Point &timestamp, $::Unique::Pointer<Action> &&action);
};
