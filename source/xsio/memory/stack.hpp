...
namespace XSIO::Memory {
    ...
    class Stack : public $_ALLOCATOR_OBJECT(XSIO_STACK) {
        //  PROPERTIES  //
       ...
       public:
         ...
         */
        constexpr Stack(size_t size = 0, size_t underflow = $::Memory::Size::KB) :
            m_buffer(size), m_underflow(underflow) {}
        ...
        inline constexpr void* high() const noexcept {
            return reinterpret_cast<void*>(reinterpret_cast<uintptr_t>(low()) + size());
        }

        /// @brief Gets the remaining stack-size of a thread.
        inline constexpr uintptr_t remaining() const noexcept {
            return std::bit_cast<uintptr_t>(__builtin_frame_address(0)) - std::bit_cast<uintptr_t>(low());
        }
