...
void $::ANSI::m_stringify(Stream::Output& os, const ANSI& self) { os << fmt::to_string(self); }
void $::ANSI::m_print(Stream::Output& os, const ANSI& self) {
    // get the current pretty enablement to be used
    if (!Dye::enabled(os)) return void(os << self.m_value);
