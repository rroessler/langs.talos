#ifndef _XTDLIB_COLOR_ANSI_HPP
#define _XTDLIB_COLOR_ANSI_HPP
...
#undef X
  ...
  protected:
     ...
     */
    static inline void m_print(std::ostream &os, const Background &self) { os << *self.m_ansi; }
  };
...
protected:
   ...
   */
  static void m_print(std::ostream &os, const ANSI &self);
};
...
#define X(N, ...)                                                                                            \
  template <class T> inline Color::ANSI N(const T &value) { return Color::ANSI(fmt::to_string(value)).N(); } \
                                                                                                             \
  template <class... As> inline Color::ANSI N(fmt::format_string<As...> message, As &&...args) {             \
    return Color::ANSI(fmt::format(message, std::forward<As>(args)...)).N();                                 \
  }                                                                                                          \
                                                                                                             \
  template <class... As> inline Color::ANSI N(const String::View &message, As &&...args) {                   \
    return Color::ANSI(fmt::format(fmt::runtime(message), std::forward<As>(args)...)).N();                   \
  }
