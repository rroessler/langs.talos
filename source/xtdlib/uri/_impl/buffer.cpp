...
$_FWD($::URI::Codec, bool escape(uint8_t))
$_FWD($::URI::Codec, URI::Buffer decode(const String::View &))
$_FWD($::URI::Codec, String::Buffer encode(const String::View &))
$_FWD($::URI::Codec, String::Buffer encode(const URI::View &))

//  PUBLIC METHODS  //

bool $::URI::Codec::escape(uint8_t ch) {
  // alphabetical and digits should be validly kept
  if (Encoding::ASCII::alpha(ch) || Encoding::ASCII::num(ch)) return false;
...
$::String::Buffer $::URI::Codec::encode(const URI::View &resource) {
  ...
  encoded += Codec::encode(resource.scheme()) + ':';
  ...
  if (!authority.empty() || body.starts_with('/')) encoded += "//" + Codec::encode(authority);

  // and return the resulting encoded value
  return encoded + Codec::encode(body);
}

//  PRIVATE METHODS  //

$::URI::Buffer $::URI::Buffer::m_decode(const Serde::Value &value) { return Codec::decode(*value.as<Serde::Text>()); }
$::Serde::Value $::URI::Buffer::m_encode(const Buffer &resource) { return Codec::encode(resource); }
