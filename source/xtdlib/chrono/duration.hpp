...
struct Duration {
  ...
  template <class R, class P> using Underlying = std::chrono::duration<R, P>;
...
public:
  ...
  /// @brief Binary "*" operator.
  friend inline constexpr Duration operator*(const Duration &left, size_t right) { return {left.m_ticks * right}; }
...
/**
 * @brief Parses a suitable multiplier.
 * @param unit                  Unit to parse.
 */
static inline constexpr int64_t Multiplier(const $::String::View &unit) {
  // prepare the output multiplier to be used
  int64_t multiplier = 1;

  // and use a switch case to suitably multiply our values
  switch (XH::FNV::U32(unit)) {
  case XH::FNV::U32("h"): $_PP_NOOP(); $_FALLTHROUGH;
  case XH::FNV::U32("hh"): multiplier *= 60; $_FALLTHROUGH;

  case XH::FNV::U32("m"): $_PP_NOOP(); $_FALLTHROUGH;
  case XH::FNV::U32("mm"): multiplier *= 60; $_FALLTHROUGH;

  case XH::FNV::U32("s"): $_PP_NOOP(); $_FALLTHROUGH;
  case XH::FNV::U32("ss"): multiplier *= 1000; $_FALLTHROUGH;

  case XH::FNV::U32(""): $_PP_NOOP(); $_FALLTHROUGH;
  case XH::FNV::U32("ms"): multiplier *= 1000; $_FALLTHROUGH;
  case XH::FNV::U32("us"): multiplier *= 1000; $_FALLTHROUGH;
  case XH::FNV::U32("ns"): return multiplier;
...
template <> struct fmt::formatter<$::Chrono::Duration, char> : fmt::formatter<$::Chrono::Ticks, char> {
  //  PUBLIC METHODS  //
