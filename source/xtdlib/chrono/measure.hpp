...
static inline constexpr struct {
  ...
  template <class R> using Callback = Unique::Functor<R()>;
   ...
   * @param callback                  Callback to measure.
   */
  template <class R> inline constexpr Result<R> apply(Callback<R> &&callback) const {
   ...
   * @param callback                  Callback to measure.
   */
  template <class F> inline constexpr Result<std::invoke_result_t<F>> apply(F &&callback) const {
