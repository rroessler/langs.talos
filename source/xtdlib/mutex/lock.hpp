...
namespace $::Lock {
    ...
    template <class M = Mutex::Auto>
    using Unique = std::unique_lock<M>;

    /// @brief Multiple Locks Alias.
    template <class... Ms>
    using Tuple = std::scoped_lock<Ms...>;
     ...
     */
    template <class M>
    static inline Unique<M> scope(M& mutex) {
        return Unique<M>(mutex);
    }
     ...
     */
    template <class... As>
    static inline Tuple<As...> join(As&&... args) {
        return Tuple<As...>(std::forward<As>(args)...);
    }
     ...
     */
    template <class M>
    static inline Mutex::Guard<M> guard(M& mutex) {
