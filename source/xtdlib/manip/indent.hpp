...
namespace $::Manip {
    ...
    struct Indent final : public std::streambuf {
        ...
        Stream::Output* m_owner;   // Owner stream instance.
       ...
       public:
         ...
         */
        explicit Indent(std::streambuf* dest, size_t level = 2) :
            m_padding(level, ' '), m_owner(nullptr), m_dest(dest) {}
         ...
         * @param level                     Current level to use.
         */
        explicit Indent(Stream::Output& owner, size_t level = 2) :
            m_padding(level, ' '), m_owner(&owner), m_dest(owner.rdbuf()) {
            m_owner->rdbuf(this);  // update the owner stream now
        }
