...
$::Filesystem::Path Shell::Upgrade::Action::m_installation() const {
...
void Shell::Upgrade::Action::m_execute() {
    ...
    XPC::Shell::Arguments argv = { "-File", script.string(), "-Tag", m_options.tag };
#else
    auto binary = "bash";  // define a suitable shell to be used
    XPC::Shell::Arguments argv = { script.string(), "--tag", m_options.tag };
#endif
    ...
    auto errc = XPC::Core::Child(builder).wait();
...
void Shell::Upgrade::Action::m_subscribe(CLI::App* command) {
    ...
    command->add_flag_callback("--remove", [&] {
        ...
        $::IO::cout() << $::Dye::green("Removing") << ": ";  // show success result
        $::IO::cout() << $::Dye::dim("Successfully uninstalled Talos!") << std::endl;
