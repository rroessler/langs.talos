...
Talos::Type::Erased Talos::Type::Dispatch::bind(Parameter *parameter, const Constraints *constraints) {
...
Talos::Type::Erased Talos::Type::Dispatch::infer(Generic *generic, const Constraints *constraints) {
...
TALOS_MM_CAPTURE_NODE(Call, node, analyzer) {
  analyzer->visit(node->callee());
  analyzer->visit(node->arguments());
}

TALOS_MM_CHECK_NODE(Call, node, analyzer) {
