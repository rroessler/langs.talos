...
TALOS_MM_FORMAT_DECL(Node, reader) {
...
Talos::Format::Piece *Talos::Format::Dispatch::m_subject(Reader *reader) {
  return m_subject(reader, reader->peek()->kind());
}

Talos::Format::Piece *Talos::Format::Dispatch::m_subject(Reader *reader, Lexer::Kind kind) {
  switch (kind) {
  case Lexer::Kind::DECL_LET: $_FALLTHROUGH;
  case Lexer::Kind::DECL_USE: $_FALLTHROUGH;
...
Talos::Format::Piece *Talos::Format::Dispatch::m_modifiers(Reader *reader, Variable::Modifiers modifiers) {
...
#define TALOS_XX_TOKEN_MODIFIER(K, N, ...)                            \
  case Lexer::Kind::MOD_##K: modifiers.set(Variable::Flag::K); break;
      TALOS_XX_TOKEN_MODIFIER(EXPORT, "export")
...
#define TALOS_XX_TOKEN_MODIFIER(N, T, ...)                                                \
