/// Talos Modules
#include "talos/type/builder.hpp"

//  PUBLIC METHODS  //

Talos::Type::Entity Talos::Type::Any::lookup(const $::String::View&) const { return m_clone(); }
Talos::Type::Entity Talos::Type::Unset::lookup(const $::String::View&) const { return m_clone(); }

Talos::Type::Erased Talos::Type::Any::apply(Operator::Kind kind) const {
    switch (kind) {
        case Operator::Kind::NOT: return Builder::boolean();
        default: return m_clone();  // default to "Any" type
    }
}

Talos::Type::Erased Talos::Type::Any::apply(Operator::Kind kind, const Erased&) const {
    switch (kind) {
        case Operator::Kind::EQ: $_FALLTHROUGH;
        case Operator::Kind::NE: $_FALLTHROUGH;
        case Operator::Kind::LOR: $_FALLTHROUGH;
        case Operator::Kind::LAND: return Builder::boolean();
        default: return m_clone();  // default to "Any" type
    }
}

void Talos::Type::None::m_print($::Stream::Output& os) const { os << Builtins::Proxy<Value::Void>::name(); }