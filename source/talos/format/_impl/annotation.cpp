...
Talos::Format::Node* Talos::Format::Dispatch::m_annotation(Reader* reader) {
    ...
    else if (reader->check(Lexer::Flag::ANNOTATION)) return m_annotation<Syntax::Qualifier>(reader);

    // failed to produce a valid annotation here so return now
    return nullptr;
}

Talos::Format::Node* Talos::Format::Dispatch::m_specialize(Reader* reader, Node* qualifier) {
    ...
    auto callback = [](Reader* reader) -> Node* { return m_annotation(reader); };
