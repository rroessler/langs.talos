...
#define TALOS_MM_FORMAT_FWD(N, T, ...) \
    template <>                        \
    Talos::Format::Node* Talos::Format::Dispatch::m_##N<Talos::Syntax::T>(__VA_ARGS__)

#define TALOS_MM_FORMAT_DECL(T, R, ...) TALOS_MM_FORMAT_FWD(declaration, T, Reader* R)
#define TALOS_MM_FORMAT_STMT(T, R, ...) TALOS_MM_FORMAT_FWD(statement, T, Reader* R)
...
#define TALOS_MM_FORMAT_INFIX(T, R, E, ...) TALOS_MM_FORMAT_FWD(infix, T, Reader* R, Node* E, bool)
