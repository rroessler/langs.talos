...
$_INLINE_FORCE Talos::Value::Any Talos::Engine::Dispatch::m_enumeration(
...
TALOS_MM_ENGINE_EXECUTE(ENUM_EMPTY, isolate, frame, instruction) {
    auto enumeration = isolate->create<Object::Enum>();  // construct
    return frame->store(instruction->get<0>(), enumeration), Mode::NEXT;
}

TALOS_MM_ENGINE_EXECUTE(ENUM_MAKE, isolate, frame, instruction) {
