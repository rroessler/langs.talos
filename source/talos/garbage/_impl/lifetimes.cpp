...
void Talos::Garbage::Lifetimes::open(Runtime::Isolate* isolate) { m_stacks[isolate->frame()].emplace_back(Scope()); }
void Talos::Garbage::Lifetimes::open(Runtime::Isolate* isolate, size_t depth) {
    // get the available stacks for this frame
    auto& stack = m_stacks[isolate->frame()];
...
void Talos::Garbage::Lifetimes::defer(Runtime::Isolate* isolate, Value::Any value) {
    // get the baseline frame as a key
    const auto* frame = isolate->frame();
...
bool Talos::Garbage::Lifetimes::m_dispose(Runtime::Isolate* isolate, Value::Any value) {
    ...
    if (!attribute.is<Function::Dynamic>()) return isolate->panic(6001000), false;

    // should be able to safely execute our incoming disposal handler
    return isolate->invoke(attribute, { value }).traits().okay();
}

void Talos::Garbage::Lifetimes::m_suppress(Runtime::Isolate* isolate, Value::Any exception) {
    if (!exception.is<Object::Exception>()) return;
    m_suppress(isolate, exception.as<Object::Exception>());
}

void Talos::Garbage::Lifetimes::m_suppress(Runtime::Isolate* isolate, Object::Exception exception) {
