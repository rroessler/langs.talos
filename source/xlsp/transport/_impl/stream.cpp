...
$_FWD(JRPC, static constexpr $::String::View KEY = "jsonrpc")
$_FWD(JRPC, static constexpr $::String::View VERSION = "2.0")
$_FWD(JRPC, static constexpr $::String::View LENGTH = "Content-Length: ")
$_FWD(JRPC, static constexpr $::String::View EOM = $_PLATFORM_WINDOWS ? "\n\n" : "\r\n\r\n")

//  PUBLIC METHODS  //

int32_t XLSP::Transport::Stream::listen(Event::Emitter *events) {
  ...
  while (!m_input.eof()) {
    ...
    auto message = $::JSON::Decode(buffer);
    ...
    else if (!*result) return std::error_code().value(); // exit
  }
...
std::expected<bool, XLSP::Error>
XLSP::Transport::Stream::m_dispatch(const $::Serde::Value &message, Event::Emitter *events) {
  ...
  auto id = *$::JSON::Encode(message.at("id"));
  ...
  if (method == nullptr) {
    ...
    return events->on_response(id, $::Serde::Decode<Error>(message.at("error")));
  }
...
void XLSP::Transport::Stream::m_send($::Serde::Object message) {
  ...
  auto buffer = *$::JSON::Encode(message);
