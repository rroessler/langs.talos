...
public:
   ...
   */
  explicit Stream(std::istream &input, std::ostream &output, const $::String::View &name = "<unknown>") :
   ...
   */
  int32_t listen(Event::Emitter *events) final;
   ...
   */
  inline void
  request(const Request::Identifier &id, const Message::Method &method, const Message::Params &params) final {
    $::Serde::Object message = {{"method", method}, {"params", params}};
    message.emplace("id", $::JSON::Decode(id).value()), m_send(message);
  }
  ...
  void respond(const Request::Identifier &id, const Message::Result<$::Serde::Value> &result) final {
    ...
    else message.emplace("error", $::Serde::Encode(std::get<Error>(result)));
    message.emplace("id", $::JSON::Decode(id).value()), m_send(message);
  }
...
private:
   ...
   */
  std::expected<bool, Error> m_dispatch(const $::Serde::Value &message, Event::Emitter *events);
   ...
   */
  template <class... As>
  inline constexpr std::expected<bool, Error> m_fail(fmt::format_string<As...> format, As &&...args) const {
    static constexpr auto code = Error::Code::PARSE_ERROR;
