...
export function Overview({ className, ...props }: Overview) {
    ...
    const portals = Registry.list().map((page) => {
        const description = new Date(page.data.date).toDateString(); // prepare the date to be used
    ...
    className = cn(className, 'mt-8 grid grid-cols-1 gap-4 text-start w-full');
