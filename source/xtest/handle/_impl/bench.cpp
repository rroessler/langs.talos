...
void XT::Bench::execute(Session::Runner* runner) const {
    ...
    if (m_meta->skip || !options->enabled) return reporter->test_skipped(this);
    ...
    try {
        ...
        auto timings = m_execute(runner, timeout, samples);
        ...
        reporter->test_failure(this, Assert::Reason::translate());
    }
}

//  PRIVATE METHODS  //

void XT::Bench::m_warmup(Session::Runner* runner, size_t iterations) const {
...
XT::Bench::Timings XT::Bench::m_execute(Session::Runner* runner, size_t timeout, const Samples& samples) const {
...
XT::Bench::Result XT::Bench::m_analyze(const Timings& timings) const {
    ...
    return { .runs = runs, .sum = sum, .mean = mean, .minimum = minimum, .maximum = maximum, .stddev = stddev };
}
