...
namespace XJCT::Archive {
    ...
    class Binary {
       ...
       public:
         ...
         */
        explicit Binary(const Buffer& buffer) : m_buffer(buffer), m_format(m_deduce(m_buffer)) {}

        /**
         * @brief Constructs a binary from an executable path.
         * @param executable            Executable to read.
         */
        explicit Binary(const $::Filesystem::Path& executable) : Binary($::Filesystem::Bytes(executable)) {}
        ...
        inline constexpr Buffer& buffer() noexcept { return m_buffer; }
        ...
        inline constexpr Blob blob() const noexcept {
            return { reinterpret_cast<const char*>(m_buffer.data()), m_buffer.size() };
        };

        /// @brief Gets the expected binary extension.
        inline constexpr $::String::View extension() const noexcept {
            return m_format == Format::WINDOWS ? ".exe" : "";
        }
       ...
       private:
         ...
         */
        static Format m_deduce(const Buffer& buffer) noexcept;
    };
